...
void CreateFileList(const std::string &path,std::vector<std::string> &vector_file_name)
{
 ...
 if (GetCurrentDirectoryA(MAX_PATH,current_directory)==FALSE) return;
 if (SetCurrentDirectoryA(path.c_str())==FALSE) return;

 WIN32_FIND_DATAA wfd;
 HANDLE handle=FindFirstFileA(static_cast<LPCSTR>("*"),&wfd);
 ...
 while(true)
 {
  ...
  if (FindNextFileA(handle,&wfd)==FALSE) break;
 }
 FindClose(handle);
 SetCurrentDirectoryA(current_directory);
}
...
{
 MessageBoxA(NULL,message.c_str(),"Сообщение",MB_OK);
}
//----------------------------------------------------------------------------------------------------
